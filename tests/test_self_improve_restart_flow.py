...
class TestSelfImproveRestartFlow(unittest.IsolatedAsyncioTestCase):
    async def test_propose_core_patch_requests_restart(self):
        ...
        engine.ctx.patches_dir = temp_dir

        with patch.object(engine, "initialize", new=AsyncMock()), \
                patch.object(engine, "validate_patch", return_value=PatchValidation(ok=True)), \
                patch.object(engine.infra, "run_test_suite", return_value=(True, temp_dir / "tests.log")), \
                patch.object(engine.comms, "notify_owner", new=AsyncMock()), \
            ...
            with patch.object(engine.infra, "agit", side_effect=fake_agit):
    ...
    async def test_startup_validation_passes_and_marks_healthy(self):
        ...
        engine.ctx.state_path = temp_dir / "state.json"
        engine.ctx.state = {
            "pending_deploy": {
                "commit_hash": "abc123",
                "healthy": False,
        ...
        with patch.object(engine, "_post_deploy_health_regression_check", new=AsyncMock(return_value=None)), \
                patch.object(engine, "_rollback_commit", new=AsyncMock()) as rollback_mock, \
                patch.object(engine.infra, "request_runtime_restart", return_value=False) as restart_mock:
            await engine._validate_pending_deploy_on_startup()

        pending = engine.ctx.state["pending_deploy"]
    ...
    async def test_startup_validation_rolls_back_and_requests_restart(self):
        ...
        with patch.object(
            ...
            engine.infra, "request_runtime_restart", return_value=True
        ) as restart_mock:
            await engine._validate_pending_deploy_on_startup()
        ...
        self.assertEqual(engine.ctx.state["pending_deploy"]["startup_validation"], "failed")
