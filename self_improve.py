...
class SelfImprovementEngine:
    ...
    async def propose_skill_from_owner_phrase(self, owner_text: str = "") -> dict[str, Any]:
        await self.initialize()
        patterns = self._detect_workflow_patterns(days=30, min_occurrences=2)
        if not patterns:
    ...
    async def propose_skill_from_patterns(
        ...
        tools = self._pattern_steps(best)
        if not tools:
    ...
    async def propose_skill_lifecycle(self, proposal: dict[str, Any]) -> dict[str, Any]:
        """Callable API for proposing a new skill or iterating an existing skill."""
        await self.initialize()

        name = str(proposal.get("name", "workflow skill")).strip() or "workflow skill"
        slug = self._slug(name) or "workflow-skill"
        ...
        trigger_values = self._normalize_skill_section_items(
            proposal.get("trigger", current_sections.get("trigger", "")),
            section="trigger",
        )
        tools_values = self._normalize_skill_section_items(
            proposal.get("tools", proposal.get("required_tools", current_sections.get("tools", ""))),
            section="tools",
        )
        steps_values = self._normalize_skill_section_items(
            proposal.get("steps", current_sections.get("steps", "")),
            section="steps",
        )
        guardrail_values = self._normalize_skill_section_items(
            proposal.get("guardrails", current_sections.get("guardrails", "")),
            section="guardrails",
        )
        ...
        draft_markdown = self._render_skill_markdown(draft_payload)
    ...
    async def _run_skill_lifecycle_approval(
        ...
        pending_path.write_text(draft_markdown)
        ...
        proposal_event_id = self._log_improvement_event(
            event_type="proposal",
            category="skill",
            title=name,
        ...
        if is_iteration:
            ...
            if decision is True:
                ...
                self._log_improvement_event(
                    ...
                    title=name,
                    payload=json.dumps(
                        {"path": str(live_path), "source": source, "fingerprint": fingerprint},
                        ensure_ascii=True,
                    ),
                    status="approved",
                )
            ...
            reason = self._extract_rejection_reason(decision)
        ...
        for _ in range(8):
            pending_path.write_text(self._render_skill_markdown(current_payload))
            simulation = self._dry_run_skill(pending_path.read_text())
            ...
            if decision is True:
                ...
                self._log_improvement_event(
                    ...
                    title=name,
                    payload=json.dumps(
                        {"path": str(live_path), "source": source, "fingerprint": current_fingerprint},
                        ensure_ascii=True,
                    ),
                    status="approved",
                )
    ...
    def _format_skill_draft_summary(
        self,
        name: str,
        pending_path: Path,
        trigger_values: list[str],
        tools_values: list[str],
        steps_values: list[str],
    ...
    def _format_skill_iteration_summary(self, name: str, pending_path: Path, section_diff: str) -> str:
        return (
    ...
    def _summarize_values(self, values: list[str], limit: int = 4) -> str:
    ...
    def _extract_skill_sections(self, markdown: str) -> dict[str, str]:
    ...
    def _normalize_skill_section_items(self, raw: Any, section: str) -> list[str]:
    ...
    def _render_skill_markdown(self, payload: dict[str, Any]) -> str:
    ...
    def _format_skill_section_diff(self, current_markdown: str, draft_markdown: str) -> str:
    ...
    def _parse_structured_skill_edits(self, instruction: str) -> dict[str, str]:
    ...
    def _apply_structured_skill_edits(self, payload: dict[str, Any], edits: dict[str, str]) -> dict[str, Any]:
        updated = {
            "name": str(payload.get("name", "workflow skill")).strip() or "workflow skill",
    ...
    def _skill_signature_from_payload(self, payload: dict[str, Any]) -> str:
        normalized = {
    ...
    def _skill_signature_from_markdown(self, markdown: str) -> str | None:
    ...
    def _skill_rejection_fingerprint(self, payload: dict[str, Any]) -> str:
        signature = self._skill_signature_from_payload(payload)
        return hashlib.sha256(signature.encode("utf-8")).hexdigest()[:24]

    def _is_skill_rejection_cooldown_active(self, fingerprint: str, days: int = 30) -> bool:
        if not fingerprint:
            return False
    ...
    def _record_skill_rejection_cooldown(
        self,
        fingerprint: str,
        title: str,
        reason: str,
    ...
    def _update_improvement_event_status(self, event_id: str | None, status: str):
        if not event_id:
            return
        try:
    ...
    async def run_weekly_assessment(self) -> Path:
        ...
        await self._propose_skill_updates_from_gap_clusters(min_cluster_size=_SKILL_GAP_MIN_CLUSTER_SIZE)
    ...
    def _log_improvement_event(
        ...
        if not logged:
            self._append_self_improvement_event_fallback(
                event_type=event_type,
                category=category,
                title=title,
                payload=payload,
    ...
    def _append_self_improvement_event_fallback(
        self,
        *,
        event_type: str,
        category: str,
    ...
    async def _propose_skill_updates_from_patterns(self, patterns: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for pattern in patterns:
            ...
            if len(steps) < 3:
                continue
            result = await self.propose_skill_from_patterns([pattern], source="pattern")
    ...
    async def _propose_skill_updates_from_gap_clusters(
        self,
        min_cluster_size: int = _SKILL_GAP_MIN_CLUSTER_SIZE,
    ) -> dict[str, Any]:
        clusters = self._query_skill_gap_clusters(min_cluster_size=min_cluster_size)
        if not clusters:
        ...
        for cluster in clusters:
            ...
            if self._has_recent_skill_gap_proposal(
                cluster_key,
            ...
            hook_result = await self._invoke_track_a_skill_hook(cluster)
            ...
            self._mark_skill_gap_rows_addressed(
                cluster_key=cluster_key,
                proposal_id=proposal_id,
    ...
    def _query_skill_gap_clusters(
        ...
        for table_name in ("skill_gap_clusters", "skill_gaps"):
            columns = self._table_columns(db_path, table_name)
            if not columns:
                continue
            ...
            rows = self._rows(db_path, f"SELECT * FROM {table_name}", ())
            for row in rows:
    ...
    def _skill_gap_row_cooldown_until(row: dict[str, Any]) -> datetime | None:
        ...
        for field in ("cooldown_until", "proposal_cooldown_until", "next_proposal_after"):
            dt = _parse_datetime(str(row.get(field, "") or ""))
            if not dt:
                continue
    ...
    def _has_recent_skill_gap_proposal(self, cluster_key: str, days: int = _SKILL_GAP_RECENT_PROPOSAL_DAYS) -> bool:
        ...
        if rows:
            try:
                ...
                pass
        return self._has_recent_event(category="skill-gap", title=f"Skill gap cluster: {cluster_key}", days=days)

    def _resolve_track_a_skill_hook(self):
        if callable(getattr(self, "_track_a_skill_gap_hook", None)):
            return getattr(self, "_track_a_skill_gap_hook")

        candidates = [
            self.molly,
            getattr(self.molly, "track_a", None) if self.molly else None,
            getattr(self.molly, "track_a_hooks", None) if self.molly else None,
            getattr(self.molly, "health_remediation", None) if self.molly else None,
        ]
        method_names = (
    ...
    async def _invoke_track_a_skill_hook(self, cluster: dict[str, Any]) -> dict[str, Any]:
        hook = self._resolve_track_a_skill_hook()
        if hook is None:
        ...
        for invoke in invocations:
            ...
            try:
                ...
                return {"status": "error", "reason": str(exc)}
            return self._normalize_track_a_skill_hook_result(result)
    ...
    def _mark_skill_gap_rows_addressed(
        self,
        *,
        cluster_key: str,
        proposal_id: str,
